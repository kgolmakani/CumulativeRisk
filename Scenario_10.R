MFP1 = read.csv("~/Desktop/MFP_split1.csv", sep=",", header=TRUE)
MFP2 = read.csv("~/Desktop/MFP_split2.csv", sep=",", header=TRUE)
MFP2$StudyID_c = as.factor(MFP2$StudyID_c)
MFP2$id1 = as.factor(MFP2$id1)
MFP2$id = as.factor(MFP2$id)
MFP3 = read.csv("~/Desktop/MFP_split3.csv", sep=",", header=TRUE)
MFP4 = read.csv("~/Desktop/MFP_split4.csv", sep=",", header=TRUE)
MFP5 = read.csv("~/Desktop/MFP_split5.csv", sep=",", header=TRUE)
MFP6 = read.csv("~/Desktop/MFP_split6.csv", sep=",", header=TRUE)
MFP7 = read.csv("~/Desktop/MFP_split7.csv", sep=",", header=TRUE)
MFP8 = read.csv("~/Desktop/MFP_split8.csv", sep=",", header=TRUE)
MFP9 = read.csv("~/Desktop/MFP_split9.csv", sep=",", header=TRUE)

dat_MFP_c_f = rbind(MFP1, MFP2, MFP3, MFP4, MFP5, MFP6, MFP7, MFP8, MFP9)

dat_MFP_c_f= dat_MFP_c_f[,-c(1,2)]

addCol = function(df){
  n = nrow(df)
  df = df[order(df$StudyID_c,df$round),]
  df$s2 = c(NA, df$s1[-n])
  df$s2[match(unique(df$StudyID_c),df$StudyID_c )] = NA 
  return (df)
}

dat_MFP_c_f = addCol(dat_MFP_c_f)

names(dat_MFP_c_f)

head(dat_MFP_c_f[,c(1,71,80, 81)], n=200)

class(dat_MFP_c_f$S) #integer

class(dat_MFP_c_f$round) #integer

class(dat_MFP_c_f$result) #numeric

class(dat_MFP_c_f$s1) #numeric

dat_MFP_c_f$result = as.factor(dat_MFP_c_f$result)

dat_MFP_c_f$s1 = as.factor(dat_MFP_c_f$s1)

dat_MFP_c_f$s2 = as.factor(dat_MFP_c_f$s2)
#examdate is the date of current screening mammogram
dat_MFP_c_f$examdate = as.Date(dat_MFP_c_f$examdate, format= "%d%b%Y")

#nxtscrdt_c is the date of next screening mammogram
dat_MFP_c_f$nxtscrdt_c = as.Date(dat_MFP_c_f$nxtscrdt_c, format= "%d%b%y")

#nxtscrdt_c is the date of next-next screening mammogram
dat_MFP_c_f$nxtscrdt_c_lead = as.Date(dat_MFP_c_f$nxtscrdt_c_lead, format= "%d%b%y")

#newdxdt is the date of woman's earliest breast cancer diagnosis
dat_MFP_c_f$newdxdt= as.Date(dat_MFP_c_f$newdxdt, format= "%d%b%Y")

#eoccadt is the date of complete cancer capture
dat_MFP_c_f$eoccadt = as.Date(dat_MFP_c_f$eoccadt, format= "%d%b%y")
...
#removing "id1","S1", "rd", "v6_AJCC_c", "v5_AJCC_c","v7_AJCC_c","ss2000_c","v7_m_c", "v7_n_c","v7_t_c"
dat_MFP_c_f = dat_MFP_c_f[,-c(72,73,74,55, 56,57,65,66,67)]

table(dat_MFP_c_f$site)
#C      D      E      I      K      A 
#20496 161707  39766  83563 156991  63593 

table(dat_MFP_c_f$examyear)
#2000  2001  2002  2003  2004  2005  2006  2007  2008  2009  2010  2011  2012  2013  2014 
#14480 17058 24608 30363 33896 39547 44798 49036 55501 60025 53221 31767 32346 27952 11518 

table(dat_MFP_c_f$result)
#0      1      2 
#436691  87061   2364 

#raw FP rate
fp_rate = (nrow(subset(dat_MFP_c_f, result==1))/ nrow(dat_MFP_c_f))*100
# the raw FP rate of the data is 16.54787% 
...
#the raw FP rate for just the first screen is 24.26747%

#we assume 10 rounds of screening (it can change)
M=10
...
dat0 = dat_MFP_c_f[, c("StudyID_c", "result")]
...
dat2$fp2 = ifelse(dat2$fp1 == M+1, M+2, dat2$fp2)# check if this matters (M+1 and M+2 should work the same)

#add fpn that is the number of false positive prior censoring
dat3 = ddply(.data= dat2, .variables = 'StudyID_c', .fun = function(df) {
  df[, 'fpn'] = sum(df$result[1:nrow(df)]==1)
  return(df)
})

table(dat3$fpn)
#     0      1      2      3      4      5      6      7 
#321994 159031  35046   7975   1653    337     62     18 

dat4 = ddply(.data=dat3, .variables='StudyID_c', .fun= function(df){
  for (i in 1:nrow(df)){
    df$v[1] = 0
    if (i > 1){df$v[i] = sum(df[1:(i-1),'result']==1)}
  }
      return(df)
})

dat = dat4[,c("fp1", "fp2", "fpn", "v")]

# Now the data set is ready that is dat_MFP_c_f
# first we use this to estimate the coefficients of the true transition matrix under the model specified in the document 
#and perform simulation using that matrix, later we use this data as the real data to estimate risk for this data

dat_MFP_c_f = cbind(dat_MFP_c_f, dat)
head(dat_MFP_c_f, n=20)


##########################################################################################################
##########################################################################################################
#extending the first scenario

set1 = subset(dat_MFP_c_f,  2<=round & round <= (S-1))

mod00 = glm(result~ 1 , data = subset(set1, s2==0 & s1==0), family=binomial)

summary(mod00)

# Coefficients:
#              Estimate Std. Error z value Pr(>|z|)    
# (Intercept) -2.46122    0.01249    -197   <2e-16 ***

mod01 = glm(result~ 1 , data = subset(set1, s2==0 & s1==1), family=binomial)

summary(mod01)


mod10 = glm(result~ 1 , data = subset(set1, s2==1 & s1==0), family=binomial)

summary(mod10)


mod11 = glm(result~ 1 , data = subset(set1, s2==1 & s1==1), family=binomial)

summary(mod11)

#------------------------------------------------------------
set2 = subset(dat_MFP_c_f,  2<=round & round ==S)

library("nnet")

model00 = multinom(result~1 , data = subset(set2, s2==0 & s1==0)) 

summary(model00)

model01 = multinom(result~1 , data = subset(set2, s2==0 & s1==1)) 

summary(model01)


model02 = multinom(result~1 , data = subset(set2, s2==0 & s1==2)) 

summary(model02)

model10 = multinom(result~1 , data = subset(set2, s2==1 & s1==0)) 

summary(model10)


model11 = multinom(result~1 , data = subset(set2, s2==1 & s1==1)) 

summary(model11)


model12 = multinom(result~1 , data = subset(set2, s2==1 & s1==2)) 

summary(model12)


model22 = multinom(result~1 , data = subset(set2, s2==2 & s1==2)) 

summary(model10)
#---------------------------------------------------------------------------------------
set0 = subset(dat_MFP_c_f, round==2)
mod10 = multinom(result~ 1 , data = subset(set0, s1==0), family=binomial)
summary(mod10)

mod11 = multinom(result~ 1 , data = subset(set0, s1==1), family=binomial)
summary(mod11)
...
P = matrix(c(1-expit(-2.46), 1-expit(-1.98),0,1-expit(-2.11), 1-expit(-1.50), 0,0,0,0,
             expit(-2.46), expit(-1.98), 0, expit(-2.11), expit(-1.50), 0, 0, 0, 0,
             0, 0,1, 0.5,0.5,1,0.5,0.5,1), ncol=3)

# P = matrix(c(1-expit(-2.46), 1-expit(-1.98),1-expit(-2.11), 1-expit(-1.50),
#              expit(-2.46), expit(-1.98), expit(-2.11), expit(-1.50), 
#              0, 0,0,0), ncol=3)

#P_prime is the transition matrix when 2<=t and t=h or t>h  
P_prime = matrix(c(1/(1+exp(-2.29)+exp(-4.70)), 1/(1+exp(-1.74)+exp(-4.05)), 0, 1/(1+exp(-1.94)+exp(-4.43)), 1/(1+exp(-1.36)+exp(-3.74)), 0,0,0,0,
                   exp(-2.29)/(1+exp(-2.29)+exp(-4.70)),exp(-1.74)/(1+exp(-1.74)+exp(-4.05)), 0, exp(-1.94)/(1+exp(-1.94)+exp(-4.43)), exp(-1.36)/(1+exp(-1.36)+exp(-3.74)), 0,0,0,0,
                   exp(-4.70)/(1+exp(-2.29)+exp(-4.70)), exp(-4.05)/(1+exp(-1.74)+exp(-4.05)),0.5, exp(-4.43)/(1+exp(-1.94)+exp(-4.43)), exp(-3.74)/(1+exp(-1.36)+exp(-3.74)), 0.5,0,0,1), ncol=3)


# P_prime = matrix(c(1/(1+exp(-2.29)+exp(-4.70)), 1/(1+exp(-1.74)+exp(-4.05)), 1/(1+exp(-1.94)+exp(-4.43)), 1/(1+exp(-1.36)+exp(-3.74)),
#                    exp(-2.29)/(1+exp(-2.29)+exp(-4.70)),exp(-1.74)/(1+exp(-1.74)+exp(-4.05)),  exp(-1.94)/(1+exp(-1.94)+exp(-4.43)), exp(-1.36)/(1+exp(-1.36)+exp(-3.74)),
#                    exp(-4.70)/(1+exp(-2.29)+exp(-4.70)), exp(-4.05)/(1+exp(-1.74)+exp(-4.05)), exp(-4.43)/(1+exp(-1.94)+exp(-4.43)), exp(-3.74)/(1+exp(-1.36)+exp(-3.74))), ncol=3)
...
round_2 = matrix(c(1/(1+exp(-2.17)+exp(-5.8)), 1/(1+exp(-1.76)+exp(-5.15)), 0,
                   exp(-2.17)/(1+exp(-2.17)+exp(-5.8)),exp(-1.76)/(1+exp(-1.76)+exp(-5.15)), 0,
                   exp(-5.8)/(1+exp(-2.17)+exp(-5.8)), exp(-5.15)/(1+exp(-1.76)+exp(-5.15)),1), ncol=3)
...
#this function returns the p_ijq ^(t,h) in the manuscript 
trnMt = function(i,j,q,h,t){ 
  
  if (2<=t & t< h)
    return(P[3*i+j+1, q+1])
  
  if (t>=h & t*h!=1)
    return(P_prime[3*i+j+1, q+1])
}
...
f0 = function(x){
  x = as.numeric(x)
  h = x[length(x)]
  c = round_1(x[1], h)*round_2[x[1]+1, x[2]+1]*Pr[h]
  for (k in 1:(length(x)-3)){ 
    c = c*trnMt(x[k],x[k+1],x[k+2], h,k+2)} 
  return (c)
}
...
MfpRisk_sim1 (100, 10, 0.2, 2, 50000, P, P_prime, 1)
...
MfpRisk_sim1 (50, 5, 0.2, 2, 50000, P, P_prime, 2)
...
################################################################
#                         scenario 10:                          #
#   false positive risk depends on the censoring time          # 
#            and is independent of screening round.            #
#    censoring time depends directly on the number of prior    #
#            false positive results                            #
#                                                              #
#            Case I : Moderate dependency                        #
################################################################

#P is the transition matrix when 2<=t<h 
P = function(v){
  matrix(c(1-expit(-1.96-0.05*(5.08+1.18*v)), 1-expit(-1.52-0.05*(3.34+1.23*v)),0,
           expit(-1.96-0.05*(5.08+1.18*v)), expit(-1.52-0.05*(3.34+1.23*v)), 0,
           0, 0, 1), ncol=3)
}

#P_prime is the transition matrix when 2<=t and t=h or t>h 
P_prime = function(v){
  matrix(c(1/(1+exp(-1.98-0.05*(3.46+1.39*v))+exp(-4.8+0.02*(3.46+1.39*v))), 1/(1+exp(-1.57-0.03*(2.07+0.9*v))+exp(-4.34+0.07*(2.07+0.9*v))), 0,
           exp(-1.98-0.05*(3.46+1.39*v))/(1+exp(-1.98-0.05*(3.46+1.39*v))+exp(-4.8+0.02*(3.46+1.39*v))),exp(-1.57-0.03*(2.07+0.9*v))/(1+exp(-1.57-0.03*(2.07+0.9*v))+exp(-4.34+0.07*(2.07+0.9*v))), 0,
           exp(-4.8+0.02*(3.46+1.39*v))/(1+exp(-1.98-0.05*(3.46+1.39*v))+exp(-4.8+0.02*(3.46+1.39*v))), exp(-4.34+0.07*(2.07+0.9*v))/(1+exp(-1.57-0.03*(2.07+0.9*v))+exp(-4.34+0.07*(2.07+0.9*v))), 1), ncol=3)
}
...
MfpRisk_sim1 (100, 10, 0.2, 2, 50000, P, P_prime, 3)
...
#P is the transition matrix when 2<=t<h 
P = function(v){
  matrix(c(1-expit(-1.96-0.1*(5.08+2*v)), 1-expit(-1.52-0.1*(3.34+2.2*v)),0,
           expit(-1.96-0.1*(5.08+2*v)), expit(-1.52-0.1*(3.34+2.2*v)), 0,
           0, 0, 1), ncol=3)
}

#P_prime is the transition matrix when 2<=t and t=h or t>h 
P_prime = function(v){
  matrix(c(1/(1+exp(-1.98-0.1*(3.46+2.5*v))+exp(-4.8+0.1*(3.46+2.5*v))), 1/(1+exp(-1.57-0.08*(2.07+1.5*v))+exp(-4.34+0.1*(2.07+1.5*v))), 0,
           exp(-1.98-0.1*(3.46+2.5*v))/(1+exp(-1.98-0.1*(3.46+2.5*v))+exp(-4.8+0.1*(3.46+2.5*v))),exp(-1.57-0.08*(2.07+1.5*v))/(1+exp(-1.57-0.08*(2.07+1.5*v))+exp(-4.34+0.1*(2.07+1.5*v))), 0,
           exp(-4.8+0.1*(3.46+2.5*v))/(1+exp(-1.98-0.1*(3.46+2.5*v))+exp(-4.8+0.1*(3.46+2.5*v))), exp(-4.34+0.1*(2.07+1.5*v))/(1+exp(-1.57-0.08*(2.07+1.5*v))+exp(-4.34+0.1*(2.07+1.5*v))), 1), ncol=3)
}
